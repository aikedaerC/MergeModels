...
class ResNet_s(nn.Module): 
    ...
    def _separate_part(self, x, ind):
        out = x # although here x.data_ptr() still equals out.data_ptr()
        out = (self.layer2s[ind])(out) # but here cal result is asigned to a new address, So, x does not change
        out = (self.layer3s[ind])(out)
        # self.feat_before_GAP.append(out)
        out = F.avg_pool2d(out, out.size()[3]) # (bs, ch, 1,1) 
