...
def _data_transforms_cifar100(args):
    ...
    if args.contrast:
        return [transforms.Compose(augmentation_weak),transforms.Compose(augmentation_strong)], valid_transform
    else:
        return transforms.Compose(augmentation_weak), valid_transform


def load_cifar100_data(args):
    train_transform, test_transform = _data_transforms_cifar100()

    cifar100_train_ds = CIFAR100_truncated(args.data_dir, train=True, download=True, transform=train_transform, contrast=args.contrast)
