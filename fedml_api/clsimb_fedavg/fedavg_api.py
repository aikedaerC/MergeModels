...
class FedAvgAPI(object):
    def __init__(self, dataset, device, args, model_trainer):
        ...
        if "esti_global" in self.args.method:
            ...
            if hasattr(self.args, "avg_similarity"):
                self.avg_similarity = self.args.avg_similarity
                self.total_esti_cls_num = self.args.total_esti_cls_num
                self.count = self.args.count
                self.esti_cls_num_list = self.args.esti_cls_num_list
            else:
        ...
        if "lade" in self.args.method or "ride" in self.args.method:
            self.experts_acc_rounds_list = []

    def _isLR(self, v1,v2):
            matrix = torch.stack([v1.view(-1),v2.view(-1)])
            rank_matrix = torch.linalg.matrix_rank(matrix)
            # 判断线性相关性
            # import pdb;pdb.set_trace()
    ...
    def train(self):
        ...
        if "global" in self.args.method:
            ...
            cls_num_list = [total_cls_count[i] if i in total_cls_count.keys() else 0.1 for i in range(self.class_num)]
        # import pdb;pdb.set_trace()
        ...
        round_range = range(self.args.start_comm_round, self.args.comm_round+1)
        ...
        for round_idx in round_range:
            ...
            if "train_exp" in self.args.method and round_idx >= int(self.args.comm_round * self.args.expert_join_time):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                if "real_global" in self.args.method:
                    ...
                    if self.total_esti_cls_num == None: # for pretraining 
                        client.set_cls_num_list(total_cls_count)
                    else:
    ...
    def server_estimate_global_distribution(self, averaged_params, global_model, sample_num, client_cls_list, idx):
        ...
        if "fc" in classifier_key:
            ...
            if self.args.num_experts > 1:
                ...
                class_dist = self._get_weight_value(local_linear - global_linear, "fc.weight", method="sum") ### LPI cal by w not gradients. !!!!!!!!!
            else:
        ...
        real_dist = np.array(client_cls_list[idx]) # shape (selected client num)

        class_dist = class_dist.numpy()  # shape (100) means selected clients' class distribution
        ...
        esti_cls_num = np.around(class_dist * sample_num).astype(np.int64)
        ...
        if self.total_esti_cls_num is None:
            ...
            if self.count == self.args.client_num_in_total:
                self.total_esti_cls_num = [i if i >= 5 else 5 for i in self.total_esti_cls_num.astype(np.int64)]
    ...
    def estimate_local_distribution(self, init_model):
        ...
        self.args.epochs = self.args.pre_epochs
        if self.args.contrast:
            self.args.method = "contrast" # "esti_global"
        else:
    ...
    def specifical_exp(self, round_idx, expert_num):
        ...
        clt_ratio_list.sort(reverse=True, key=lambda x: x[0]) # 根据每个类中实例数量由大到小排序

        clts_sorted_by_cls = [i[1] for i in clt_ratio_list] # 类标签，按照实例数量从大到小的顺序
        # logging.info("###########clts_sorted_by_cls########\n" + str(clts_sorted_by_cls))
        expert_clt_idxs = []
        for i in range(expert_num): # 平均分配给expert, 3个expert分别对应many-shot, medium-shot, few-shot
        ...
        for i in range(expert_num):
            if self.args.client_num_per_round * self.args.beta >= len(expert_clt_idxs[i]):
                ...
                clt_indexes = np.random.choice(expert_clt_idxs[i], int(self.args.client_num_per_round * client_ratio[i]), replace=False) # 组内选择beta比例
                ...
                random_clt = np.random.choice(range(self.args.client_num_in_total), int(self.args.client_num_per_round * (1-client_ratio[i])), replace=False) # 全局选择1-beta比例

            clt_indexes = np.concatenate((clt_indexes, random_clt)) # 分别对应组内，组间
    ...
    def _global_test(self, round_idx, total_cls_count):
        ...
        if "imagenet" in self.args.dataset:
            ...
            train_local_metrics, batch_loss = self.model_trainer.test(self.train_global, round_idx, total_cls_count, self.device, self.args)
        ...
        test_local_metrics, batch_loss = self.model_trainer.test(self.test_global, round_idx, total_cls_count, self.device, self.args)
        ...
        if not self.args.debug and self.args.contrast:
            wandb.log({"TotalLoss": batch_loss["total_loss"], "round": round_idx})
            wandb.log({"PerformLoss": batch_loss["perform_loss"], "round": round_idx})
            wandb.log({"RouteweightLoss": batch_loss["routeweight_loss"], "round": round_idx})
            wandb.log({"ReverseLoss": batch_loss["reverse_loss"], "round": round_idx})
        ...
        if "esti_global" in self.args.method:
            self.args.avg_similarity = self.avg_similarity
            self.args.total_esti_cls_num = self.total_esti_cls_num
